    ...
    if lang_flag == '-br':
        ...
        if not os.path.exists(br_path) and os.path.exists(bt_path):
            try:
                ...
                print(f"[info] Copied {bt_path} -> {br_path} to meet '-br'.")
            except Exception as e:
                print(f"[warn] Copying failed {bt_path} to {br_path}: {e}")
...
def generate_mnemonic():
    try:
        ...
        # Errors returned by the C binary (exitcode != 0)
        ...
        # Any other Python error|Qualquer outro erro Python
